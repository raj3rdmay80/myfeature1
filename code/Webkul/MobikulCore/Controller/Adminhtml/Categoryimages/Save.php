...
class Save extends \Webkul\MobikulCore\Controller\Adminhtml\Categoryimages
{
    ...
    public function execute()
    {
        ...
        if ($originalRequestData) {
            try {
                ...
                if (isset($featuredcategoriesData['category_id']) &&
                    ($featuredcategoriesData['category_id'] == '' ||
                    $featuredcategoriesData['category_id'] == 0)
                ) {
                ...
                $categoryimagesData["smallbanner"] = $this->uploadAndSaveSmallbannerImage();
    ...
    public function uploadAndSaveBannerImage()
    {
        ...
        $rendered = $this->getRequest()->getParam("mobikul_categoryimages_banner_rendered") ?? 0;
        if (!isset($exist)) {
            $exist = [];
        }
     ...
     * @return string
     */
    public function uploadAndSaveSmallbannerImage()
    {
        $files = $this->getRequest()->getFiles();
        $exist = $this->getRequest()->getParam("mobikul_categoryimages_small_exist");
        $rendered = $this->getRequest()->getParam("mobikul_categoryimages_smallbanner_rendered") ?? 0;
        if (!isset($exist)) {
            $exist = [];
        }
        $storeId = $this->getRequest()->getParam("store_id");
        $path = $this->mediaDirectory->getAbsolutePath("mobikul/categoryimages/smallbanner/");
        $postParams = $this->getRequest()->getParam("mobikul_categoryimages");
        if (isset($postParams["id"]) && $postParams["id"] > 0) {
            $categoryId = $postParams["id"];
        } else {
        ...
        if ($categoryId) {
            $currentCategoryData = $this->categoryimagesRepository->getById($categoryId);
            if ($currentCategoryData->getSmallbanner()) {
                $currentCategoryBannerimages = explode(",", $currentCategoryData->getSmallbanner());
            } else {
                $currentCategoryBannerimages = [];
            }
        } else {
        ...
        $hasFileToUpload = false;
        if (!empty($files["mobikul_categoryimages_smallbanner"])) {
            $keys = array_keys($files["mobikul_categoryimages_smallbanner"]);
            foreach ($keys as $value) {
                if ($files["mobikul_categoryimages_smallbanner"][$value]["size"] > 0) {
                    $hasFileToUpload = true;
                }
            }
        }
        ...
        if (!empty($files["mobikul_categoryimages_smallbanner"])) {
            ...
            foreach ($keys as $value) {
                if ($files["mobikul_categoryimages_smallbanner"][$value]["size"] > 0) {
                    try {
                        $uploader = $this->fileUploaderFactory->create(
                            ["fileId" => "mobikul_categoryimages_smallbanner[".$value."]"]
                        );
                        $fileName = $files["mobikul_categoryimages_smallbanner"][$value]["name"];
                        $ext = substr($fileName, strrpos($fileName, ".") + 1);
                        $editedFileName = "File-".$time.".".$ext;
                        $uploader->setAllowedExtensions(["png", "jpg", "jpeg"]);
                        $uploader->setAllowRenameFiles(true);
