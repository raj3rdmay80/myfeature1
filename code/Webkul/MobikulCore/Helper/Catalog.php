...
class Catalog extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    protected function isProductNew($product)
    {
        ...
        if (is_null($productNewToDate) && is_null($productNewFromDate)) {
            return false;
        }
        if (strtotime($todayStartOfDayDate) >= strtotime($productNewFromDate)
            ...
            return true;
        } elseif (strtotime($todayStartOfDayDate) >= strtotime($productNewFromDate) && is_null($productNewToDate)) {
            return true;
        } elseif(strtotime($todayEndOfDayDate) <= strtotime($productNewToDate) && is_null($productNewFromDate)) {
            return true;
        } else {
            return false;
        }
    ...
    public function getPriceFilter($priceFilterModel, $storeId)
    {
        if ($this->getPriceRangeCalculation() == "improved") {
            $algorithmModel = $this->_objectManager->create(\Magento\Catalog\Model\Layer\Filter\Price\Algorithm::class);
