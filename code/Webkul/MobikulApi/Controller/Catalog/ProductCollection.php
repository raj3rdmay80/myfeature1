...
class ProductCollection extends AbstractCatalog
{
    ...
    protected function getSortingData()
    {
        ...
        $availableSortBy = [];
        if ($this->type == "category") {
            $availableSortBy = $this->loadedCategory->getAvailableSortBy();
        }
        foreach ($toolbar->getAvailableOrders() as $key => $order) {
            if (count($availableSortBy) > 0 && !in_array($key, $availableSortBy)) {
                continue;
            }
            $each = [];
            $each["code"] = $key;
    ...
    protected function sortProductCollection()
    {
        $defaultSortBy = '';
        if ($this->type == "category") {
            $defaultSortBy = $this->loadedCategory->getDefaultSortBy();
        }
        ...
        if (count($this->sortData) > 0) {
            ...
            if ($this->sortData[1] == 0) {
                $this->setCollectionOrder($isFlatEnabled, $sortBy, "ASC");
            } else {
                $this->setCollectionOrder($isFlatEnabled, $sortBy, "DESC");
            }
        } else {
            if ($this->collection) {
                if ($defaultSortBy != '') {
                    $this->setCollectionOrder($isFlatEnabled, $defaultSortBy, "ASC");
                } else {
                    $this->setCollectionOrder($isFlatEnabled, "position", "ASC");
                }
            }
        }
    }
     ...
     *
     * @param boolean $isFlatEnabled
     * @param string $sortBy
     * @param string $sortOrder
     * @return void
     */
    protected function setCollectionOrder($isFlatEnabled, $sortBy, $sortOrder) {
        if ($isFlatEnabled) {
            $this->collection->setOrder($sortBy, $sortOrder);
        } else {
            $this->collection->addAttributeToSort($sortBy, $sortOrder);
        }
    }
