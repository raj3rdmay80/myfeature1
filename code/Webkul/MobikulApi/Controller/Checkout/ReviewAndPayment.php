    ...
    protected function getPaymentMethods($quote)
    {
        ...
        foreach ($paymentDetails["payment_methods"] as $method) {
            ...
            if (in_array($method->getCode(), ["paypal_standard", "paypal_express"])) {
                ...
                $oneMethod["extraInformation"] = $extraInformation;
            } elseif ($method->getCode() == "razorpay") {
                $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
                $curl = $objectManager->create(\Magento\Framework\HTTP\Client\Curl::class);
                $url = "https://api.razorpay.com/v1/orders";
                $authKey = $this->helper->getConfigData("payment/razorpay/key_id");
                $authSecret = $this->helper->getConfigData("payment/razorpay/key_secret");
                $headers = [
                ...
                $curl->setCredentials($authKey, $authSecret);
                $fields = [
                    "amount" => (int)$amount,
                    "currency" => $quote->getCurrency()->getQuoteCurrencyCode(),
                    "payment_capture" => 1
                ];
                $curl->post($url, json_encode($fields));
                $result = $curl->getBody();

                if ($this->isJson($result)) {
                    $result = json_decode($result);
                    if(isset($result->id)){
                        $eachData['razorpay_order_id'] = $result->id;
                    }
                }
                $eachData['amount'] = (int)$amount;
                $eachData['razorpay_title'] = $this->helper->getConfigData("payment/razorpay/title");
                $eachData['razorpay_merchantName'] = $this->helper->getCOnfigData("payment/razorpay/merchant_name_override");
                $eachData['razorpay_enable'] = $this->helper->getCOnfigData("payment/razorpay/active");
                $eachData['razorpay_action'] = $this->helper->getCOnfigData("payment/razorpay/payment_action");
                $eachData["razorpay_keyId"] = $this->helper->getConfigData("payment/razorpay/key_id");
                $eachData["razorpay_KeySecret"] = $this->helper->getConfigData("payment/razorpay/key_secret");
                $eachData["razorpay_webhook"] = $this->helper->getConfigData("payment/razorpay/enable_webhook");
                $eachData["razorpay_webhookSecret"] = $this->helper->getConfigData("[ayment/razorpay/webhook_secret");
                $eachData['customer_phone_number'] = $quote->getShippingAddress()->getTelephone();
                $eachData['customer_name'] = $quote->getShippingAddress()->getFirstname();
                $eachData['customer_lastname'] = $quote->getShippingAddress()->getLastname();
                $eachData['customer_email'] = $quote->getShippingAddress()->getEmail();
                $this->checkoutSession->setOrderAmount((int)$amount);
                $this->returnArray["razorpay_data"] = $eachData;
            }
