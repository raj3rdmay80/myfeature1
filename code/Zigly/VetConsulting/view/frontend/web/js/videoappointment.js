...
define([
    ...
    return function(config) {
        ...
        $(document).on('click', '.next-nav', function() {
            $("#errormsg").remove();
            if ($('.current').hasClass("field2")) { //pet
        ...
        $(document).on('click', '.btn-payment-make', function() {
            ...
            $.ajax({
                url: urlBuilder.build('services/video/place'),
                method: "POST",
                data: placeData,
                showLoader: true,
                success: function(response) {
                    if (response.success) {
                        if (response.razorData) {
                            var options = {
                                ...
                                "callback_url": urlBuilder.build('services/video/paynow')+'/?id='+response.razorData.orderId,
                                "prefill": {
                ...
                if (clickedDay >= from && clickedDay <= to) {
                    ...
                    if (todayString == dateString && parseInt(moment(today).format('H')) > (parseInt(config.startHr) - 2)) {
                        ...
                        if ((parseInt(currentHr) == endHr && parseInt(currentMin) >= endMin) || parseInt(currentHr) > endHr) {
                            $('<div class="message-notice notice message" id="errormsg">Time-slot are unavailable for the Selected day.</div>').insertAfter('#grooming-timeslot ol');
