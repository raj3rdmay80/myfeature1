...
class Getplans extends \Magento\Framework\App\Action\Action
{
    ...
    public function execute()
    {
        ...
        $block = $resultPage->getLayout()
                ->createBlock('Zigly\GroomingService\Block\ExperienceGrooming\Plan')
                ->setTemplate('Zigly_GroomingService::experiencegrooming/plans.phtml')
                ->setData('petData', $post)
                ->toHtml();

        $result->setData(['output' => $block,'FF'=>"DATA", 'pet_name' => $petName]);
