<?php
/**
 * Copyright (C) 2020  Zigly
 * @package  Zigly_GroomingService
 */
declare(strict_types=1);

namespace Zigly\GroomingService\Controller\Experiencegrooming;

use Magento\Customer\Model\Customer;
use Zigly\GroomingService\Model\Session;
use Magento\Framework\App\Action\Context;
use Zigly\Wallet\Helper\Data as WalletHelper;
use Magento\Framework\View\Result\PageFactory;
use Magento\Framework\Controller\Result\JsonFactory;
use Magento\Customer\Model\SessionFactory as CustomerSession;
use Zigly\CouponService\Model\ResourceModel\CouponService\CollectionFactory as CouponCollection;

class Applywallet extends \Magento\Framework\App\Action\Action
{

    /**
     * Customer session model
     *
     * @var CustomerSession
     */
    protected $customerSession;

    /**
     * @var JsonFactory
     */
    protected $jsonResultFactory;

    /**
     * @var CouponCollection
     */
    protected $couponCollection;

    /**
     * @var Session
     */
    protected $groomingSession;

    /**
     * @var WalletHelper
     */
    private $walletHelper;

    /**
     * Constructor
     *
     * @param Context  $context
     * @param Customer $customer
     * @param Session $groomingSession
     * @param WalletHelper $walletHelper
     * @param JsonFactory $jsonResultFactory
     * @param CustomerSession $customerSession
     * @param CouponCollection $couponCollection
     */
    public function __construct(
        Context $context,
        Customer $customer,
        Session $groomingSession,
        WalletHelper $walletHelper,
        JsonFactory $jsonResultFactory,
        CustomerSession $customerSession,
        CouponCollection $couponCollection
    ) {
        $this->customer = $customer;
        $this->walletHelper = $walletHelper;
        $this->customerSession = $customerSession;
        $this->groomingSession = $groomingSession;
        $this->couponCollection = $couponCollection;
        $this->jsonResultFactory = $jsonResultFactory;
        parent::__construct($context);
    }

    /**
     * Execute save action
     * @return void
     */
    public function execute()
    {
        $post = $this->getRequest()->getPostValue();
        $result = $this->jsonResultFactory->create();
        /*$resultPage = $this->resultPageFactory->create();*/
        $responseData = [
            'success' => false,
            'message' => 'Insufficient balance in wallet money.'
        ];
        try {
            $customer = $this->customerSession->create()->getCustomer();
            $totalBalance = is_null($customer->getWalletBalance()) ? "0" : $customer->getWalletBalance();
            if ($this->walletHelper->isEnabled() && $totalBalance > 1 ) {
                $groomSession = $this->groomingSession->getGroomCenter();
                $maxTransactionPercent = (!empty($this->walletHelper->getMaxTransactionPercent())) ? (float)$this->walletHelper->getMaxTransactionPercent() : 0;
                $subtotal =  floor($groomSession['subtotal']);
                if ($groomSession['coupon_discount_amount'] > 0) {
                    $subtotal = $subtotal -  floor($groomSession['coupon_discount_amount']);
                }
                $calculatedWalletApplicable = ($maxTransactionPercent / 100) * $subtotal;
                if ($calculatedWalletApplicable <= 0) {
                    $groomSession['grand_total'] = 0;
                } else {
                    if ($totalBalance <= $calculatedWalletApplicable) {
                        $groomSession['grand_total'] =  floor($totalBalance);
                    } else if ($totalBalance > $calculatedWalletApplicable) {
                        $groomSession['grand_total'] =  floor($calculatedWalletApplicable);
                    }
                }
                $groomSession['discount_amount'] = ($subtotal > $groomSession['grand_total']) ? $groomSession['grand_total'] : $subtotal;
                $groomSession['wallet_discount_amount'] = ($subtotal > $groomSession['grand_total']) ? $groomSession['grand_total'] : $subtotal;
                $groomSession['wallet_amount'] = ($subtotal > $groomSession['grand_total']) ? $groomSession['grand_total'] : $subtotal;
                $grandTotal = $subtotal - $groomSession['grand_total'];
                $groomSession['grand_total'] =   floor($grandTotal);
                $groomSession['wallet'] =   floor($grandTotal);
                if ($groomSession['coupon_discount_amount'] > 0) {
                    $groomSession['wallet'] =  1;
                }
                $responseData['success'] = true;
                $responseData['amount'] = $grandTotal;
                $responseData['discount'] = $groomSession['wallet_discount_amount'];
                $responseData['message'] = "Successfully applied.";
                $this->groomingSession->setGroomCenter($groomSession);
            }
        } catch (\Exception $e) {
            $responseData = [
                'success' => false,
                'trace' => $e->getMessage(),
                'message' => $e->getMessage()
            ];
        }
        $result->setData($responseData);
        return $result;
    }
}