...
class Index extends \Magento\Framework\View\Element\Template
{
     ...
     * @param ResourceConnection $resourceConnenction
    ...
    public function __construct(
        ...
        ResourceConnection $resourceConnenction,
        ...
        $this->resourceConnenction = $resourceConnenction;
    ...
    public function getAddressCollection()
    {
        ...
        // $pinCheck = $this->cookieManager->getCookie('pincode_check');
        $latlng = $this->cookieManager->getCookie('glatlng');
        $lat = '';
        if ($latlng) {
        ...
        if ($lat) {
            $distanceInM = (8 * 2) * 1000;
            $connection = $this->resourceConnenction->getConnection();
            $query = 'SELECT `main_table`.*, st_distance_sphere(POINT(longitude,latitude), POINT('.(float)$lng.','.(float)$lat.'))as distance FROM `zigly_hub_hub` AS `main_table` WHERE (`status` = 1 AND `city` ';
            if (!empty($city) && (strcasecmp($city, 'New delhi') == 0)) {
                $query .= 'IN ("New Delhi", "Delhi")';
            } else {
                $query .= '= "'.$city.'"';
            }
            $query .=') AND ((st_distance_sphere(POINT(longitude,latitude), POINT('.(float)$lng.','.(float)$lat.')) < '.$distanceInM.') OR (`main_table`.latitude  IS NULL AND `main_table`.longitude  IS NULL)) Order by distance asc';
            $queryData =  $connection->fetchAll($query);
            return $queryData;
        }
        // AND (`pincode` LIKE "%'.$pinCheck.'%")

        $address =  $this->hub->create()->getCollection();
        if (!empty($city) && (strcasecmp($city, 'New delhi') == 0)) {
            $address->addFieldToFilter('city', ['in' => ['New Delhi', 'Delhi']]);
        } else {
            $address->addFieldToFilter('city', ['eq' => $city]);
        }
        $address->addFieldToFilter('status', ['eq' => 1]);
        $address->getData();
        // ->addFieldToFilter('pincode', ['like' => '%'.$pinCheck.'%'])
        return $address;
    }
