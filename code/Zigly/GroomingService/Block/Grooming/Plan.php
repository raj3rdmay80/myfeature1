...
class Plan extends \Magento\Framework\View\Element\Template
{
    ...
    public function getGroomingSession()
    {
        if (!$this->groomSession) {
            $this->groomSession = $this->groomingSession->getGroomService();
        }
     ...
     * @return []
     */
    public function getLastBookedPlan()
    {
        $lastPlan = false;
        $customerId = $this->customer->getCustomerId();
        $groomBookingCollection = $this->groomingCollectionFactory->create()->addFieldToSelect('planid')->addFieldToFilter('customer_id', ['eq' => $customerId])->setOrder('created_at', 'DESC')->setPageSize(1)->getData();
        if (count($groomBookingCollection) && !empty($groomBookingCollection[0]['planid'])) {
            // active
            // breed
            // city
            // athome
            // $lastPlan = $this->plans->create()->getItemByColumnValue('plan_id', $groomBookingCollection[0]['planid']);
        }
        return $lastPlan;
    }
