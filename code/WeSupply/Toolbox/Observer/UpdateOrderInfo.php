...
class UpdateOrderInfo implements ObserverInterface
{
    ...
    public function execute(Observer $observer)
    {
        ...
        if (
            $this->helper->shouldIgnoreOrder($order) ||
            ($order->getStatus() === 'pending' && $this->helper->excludePendingOrders())
        ) {
            /**
             * no recording or updates for order
             * if it match the filters of 'Orders To Be Exported/Updated' section
             */
            return $this;
        }
        ...
        try {
            ...
            if ($this->request->getFrontName() === 'admin' /** check only for updates from magento admin */
                && (
                    /** in case of order update for order that was excluded at the time when it was placed */
                    (!$weSupplyOrder->getId() && !$order->getExcludeImportPending())
