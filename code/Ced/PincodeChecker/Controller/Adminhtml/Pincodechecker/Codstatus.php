 ...
 */
class Codstatus extends \Magento\Backend\App\Action
{
    /**
    ...
    public function execute()
    {
        ...
        $can_cod = $this->getRequest()->getParam('cod_status');
        if (!is_array($ids)) {
            ...
            if (!empty($ids)) {
                try {
                    foreach ($ids as $id) {
                        $this->pincodeFactory->create()
                            ->load($id)
                            ->setData('can_cod', $can_cod)
                            ->save();
                    }
                    $this->messageManager->addSuccessMessage(__('Total of %1 record(s) have been updated.', count($ids)));
                } catch (\Exception $e) {
