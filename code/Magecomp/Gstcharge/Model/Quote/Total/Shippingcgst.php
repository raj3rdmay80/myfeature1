...
class Shippingcgst extends AbstractTotal
{
    ...
    )
    {
		...
		{ 
				...
				foreach ($quote->getAllVisibleItems() as $item) 
				{
				 ...
				 if($this->helperData->getShippingGstTaxType())
				 {
				...
				if($this->helperData->getShippingGstTaxType())
				{
					$total->setTotalAmount('shipping_cgst_charge', $shippingGst);
					$total->setBaseTotalAmount('shipping_cgst_charge', $shippingGst);
					$total->setBaseGrandTotal($total->getBaseGrandTotal() + $address->getShippingCgstCharge());
				}else{
    ...
    public function fetch(Quote $quote, Total $total)
    {
        ...
        $fee = $this->helperData->getShippingCgstCharge();
        $result = array();
        if ($enabled && $isShippingEnabled) {
            $result = [
    ...
    {
        return __('shipping_cgst_charge Fee');
    }
